...
The default Windows Event Logs forwarded by the Wazuh agent provide limited insight into endpoint activity. Without Sysmon, many logs generated by Windows leave little or no trace, making analysis of logs less insightful. Sysmon fills this gap by generating more detailed telemetry at the system level, including:
...
This additional telemetry is essential for detecting and investigating security incidents conducted in this lab and gathering experience with Sysmon, which is commonly deployed alongside SIEM agents in real enterprise environments.

## Download

Sysmon is part of the Microsoft Sysinternals suite. For this lab, the most recent Sysmon version 15.15 was used, but the latest version can be downloaded directly from the [Microsoft Sysinternals official download page](https://learn.microsoft.com/en-us/sysinternals/downloads/sysmon). The zip archive contains four files:
